...
function checkMission() {
    ...
    if (isNotKillGame && actorsToNotKill.some(actor => currentLevel.kills[actor.type] >= actor.toNotKill)) {
    ...
    if (isKillGame) {
        successChecks.push(actorsToKill.every(actor => currentLevel.kills[actor.type] >= actor.toKill));
    }
    
    if (isSaveGame) {
        successChecks.push(actorsToSave.every(actor => currentLevel.alive[actor.type] >= actor.toSave));
    }
    
    if (!currentLevel.disableEarlySuccess && successChecks.length > 0) {

        const levelComplete = successChecks.every(successCheck => successCheck === true);
        if (levelComplete) {
