...
const getRandomRange = randomRange =>
  Math.random() * randomRange - randomRange * 0.5;
...
class Player {
  ...
  spawn() {
    ...
    this.cameraSprite = this.game.add.sprite(
  ...
  fireLaser() {
    ...
    if (laser) {
      laser.reset(this.sprite.centerX, this.sprite.centerY);
  ...
  updateThrust() {
    ...
    if (this.keyboard.cursorUp.isDown) {
      const timeThrusting =
        this.game.time.time - this.keyboard.cursorUp.timeDown;
  ...
  updateAcceleration(delta) {
    ...
    if (this.joystick.properties.inUse) {
      this.sprite.body.acceleration.x =
        PLAYER.POWER * this.joystick.properties.x;
      this.sprite.body.acceleration.y =
        PLAYER.POWER * this.joystick.properties.y;
      return;
    }
