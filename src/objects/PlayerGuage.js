...
class PlayerGuage {
    ...
    renderSprites(){
        this.guageSprite = this.game.add.sprite(this.offset.x, this.offset.y, 'guage');
        this.guageSprite.fixedToCamera = true;
        this.guageSprite.anchor.set(0.5);
        this.needleSprite = this.game.add.sprite(this.offset.x, this.offset.y, 'guage_needle');
	...
	update(percentage){
        ...
        // this.needleSprite.angle = newAngle;
        this.game.add.tween(this.needleSprite).to({
            angle: newAngle
        }, 200,
        null, true, 0, 1);
